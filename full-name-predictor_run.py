...
def read_csv():
    ...
    #     surnames = csvfile.readlines()
    # surnames = [x.split(',')[0] for x in surnames]
...
def check_middle_name(first_name, second_name):
    ...
    # second_second_name = second_name.split(' ')[1]

    first_length = len(first_name.split(' '))
    # first_last_name = first_name.split(' ')[first_length - 1]
    # first_first_name = first_name.split(' ')[0]
...
def get_accuracy(names):
    accuracy = 0
    i = 0
    with open('./dev-key.csv', newline='') as csvfile:
        spamreader = csv.reader(csvfile, quotechar='|')
        for row in spamreader:
            expected = row[1]
            if expected == names[i]:
                ...
                # print("first_name: " + row[0].split(" AND ")[0])
                # print("second_name: " + row[0].split(" AND ")[1])
                # print("Answer: " + names[i])
                # print("Expected: " + expected)
                # print('\n')
                print(row[0].split(" AND ")[0] + "             " + row[0].split(" AND ")[1] + "             " + names[
                    i] + "            " + expected)
            i += 1
